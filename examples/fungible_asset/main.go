...
func example(networkConfig client.NetworkConfig) {
	...
	sender, err := types.NewAccountFromSigner(&key)
	if err != nil {
	...
	faMetadataAddress := &types.AccountAddress{}
	...
	amount, err := bcs.SerializeU64(10000000)
	if err != nil {
	...
	transferTxn, err := faClient.TransferPrimaryStore(sender, *receiver, 100)
	if err != nil {
		panic("Failed to create primary store transfer transaction:" + err.Error())
	}
	response, err = aptosClient.SubmitTransaction(transferTxn)
	if err != nil {
		panic("Failed to submit transaction:" + err.Error())
	}
	fmt.Printf("Submitted transfer as: %s\n", response.Hash)
	err = aptosClient.PollForTransactions([]string{response.Hash})
	if err != nil {
