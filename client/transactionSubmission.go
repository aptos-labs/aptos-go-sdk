...
// BuildTransactions start a goroutine to process [types.TransactionPayload] and spit out [RawTransactionImpl].
func (client *Client) BuildTransactions(sender types.AccountAddress, payloads chan TransactionBuildPayload, responses chan TransactionBuildResponse, setSequenceNumber chan uint64, options ...any) (*types.RawTransaction, error) {
	return client.BuildTransactions(sender, payloads, responses, setSequenceNumber, options...)
}

// BuildTransactions start a goroutine to process [types.TransactionPayload] and spit out [RawTransactionImpl].
func (rc *NodeClient) BuildTransactions(sender types.AccountAddress, payloads chan TransactionBuildPayload, responses chan TransactionBuildResponse, setSequenceNumber chan uint64, options ...any) {
	// Initialize state
...
func (rc *NodeClient) BatchSubmitTransactions(requests chan TransactionSubmissionRequest, responses chan TransactionSubmissionResponse) {
	...
	inputs := make([]*types.SignedTransaction, 20)
	ids := make([]uint64, 20)
	i := uint32(0)
...
func (client *Client) BuildSignAndSubmitTransactions(
	sender types.TransactionSigner,
...
func (rc *NodeClient) BuildSignAndSubmitTransactions(
	...
	singleSigner := func(rawTxn types.RawTransactionImpl) (*types.SignedTransaction, error) {
		switch rawTxn := rawTxn.(type) {
		case *types.RawTransaction:
			return rawTxn.SignedTransaction(sender)
		case *types.RawTransactionWithData:
			switch rawTxn.Variant {
			case types.MultiAgentRawTransactionWithDataVariant:
				return nil, fmt.Errorf("multi agent not supported, please provide a signer function")
			case types.MultiAgentWithFeePayerRawTransactionWithDataVariant:
...
//		feePayer := types.NewEd25519Account()
//
//		payloads := make(chan TransactionBuildPayload)
//		responses := make(chan TransactionSubmissionResponse)
//
//		signingFunc := func(rawTxn RawTransactionImpl) (*types.SignedTransaction, error) {
//			switch rawTxn.(type) {
//			case *types.RawTransaction:
...
func (client *Client) BuildSignAndSubmitTransactionsWithSignFunction(
	...
	payloads chan TransactionBuildPayload,
	responses chan TransactionSubmissionResponse,
	sign func(rawTxn types.RawTransactionImpl) (*types.SignedTransaction, error),
	buildOptions ...any,
) {
