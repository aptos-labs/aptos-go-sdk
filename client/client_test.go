...
type CreateSigner func() (types.TransactionSigner, error)

var TestSigners map[string]CreateSigner

type CreateSingleSignerPayload func(client *Client, sender types.TransactionSigner, options ...any) (*types.RawTransaction, error)
...
func initSigners() {
	...
	TestSigners["Standard Ed25519"] = func() (types.TransactionSigner, error) {
		signer, err := types.NewEd25519Account()
		return any(signer).(types.TransactionSigner), err
	}
	TestSigners["Single Sender Ed25519"] = func() (types.TransactionSigner, error) {
		signer, err := types.NewEd25519SingleSignerAccount()
		return any(signer).(types.TransactionSigner), err
	}
	TestSigners["Single Sender Secp256k1"] = func() (types.TransactionSigner, error) {
		signer, err := types.NewSecp256k1Account()
		return any(signer).(types.TransactionSigner), err
	}
	TestSigners["2-of-3 MultiKey"] = func() (types.TransactionSigner, error) {
		signer, err := NewMultiKeyTestSigner(3, 2)
		return any(signer).(types.TransactionSigner), err
	}
	/* TODO: MultiEd25519 is not supported ATM
	TestSigners["MultiEd25519"] = func() (types.TransactionSigner, error) {
...
func initSingleSignerPayloads() {
	...
	TestSingleSignerPayloads["types.Script"] = buildSingleSignerScript
}
...
func setupIntegrationTest(t *testing.T, createAccount CreateSigner) (*Client, types.TransactionSigner) {
...
func Test_Concurrent_Submission(t *testing.T) {
	...
	account1, err := types.NewEd25519Account()
	assert.NoError(t, err)
	account2, err := types.NewEd25519Account()
	assert.NoError(t, err)
	...
	transferPayload, err := types.CoinTransferPayload(nil, types.AccountOne, 100)
	assert.NoError(t, err)
	...
	for i := uint64(0); i < numTxns; i++ {
		payloads <- TransactionBuildPayload{
			...
			Inner: types.TransactionPayload{
...
func TestClient_View(t *testing.T) {
	...
	payload := &ViewPayload{
		Module:   types.ModuleId{Address: types.AccountOne, Name: "coin"},
		Function: "balance",
		ArgTypes: []types.TypeTag{types.AptosCoinTypeTag},
		Args:     [][]byte{types.AccountOne[:]},
	}
...
func buildSingleSignerEntryFunction(client *Client, sender types.TransactionSigner, options ...any) (*types.RawTransaction, error) {
	return APTTransferTransaction(client, sender, types.AccountOne, 100, options...)
}

func buildSingleSignerScript(client *Client, sender types.TransactionSigner, options ...any) (*types.RawTransaction, error) {
	scriptBytes, err := util.ParseHex(singleSignerScript)
	if err != nil {
		return nil, err
	}
	...
	rawTxn, err := client.BuildTransaction(sender.AccountAddress(),
		types.TransactionPayload{Payload: &types.Script{
			Code:     scriptBytes,
			ArgTypes: []types.TypeTag{},
			Args: []types.ScriptArgument{{
				Variant: types.ScriptArgumentU64,
				Value:   amount,
			}, {
				Variant: types.ScriptArgumentAddress,
				Value:   dest,
			}},
		}},
		options...,
